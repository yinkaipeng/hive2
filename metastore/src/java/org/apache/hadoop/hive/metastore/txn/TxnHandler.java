    ...
    synchronized (TxnHandler.class) {
      if(connPool == null) {
        ...
        try {
          ...
          long getCommectionTimeoutMs = 30000;
          connPool = setupJdbcConnectionPool(conf, maxPoolSize, getCommectionTimeoutMs);
          ...
          connPoolMutex = setupJdbcConnectionPool(conf, maxPoolSize + MUTEX_KEY.values().length, getCommectionTimeoutMs);
...
\         */
  ...
  private static synchronized DataSource setupJdbcConnectionPool(HiveConf conf, int poolSize, long getConnectionTimeoutMs) throws SQLException {
    ...
    if ("bonecp".equals(connectionPooler)) {
      ...
      //if we are waiting for a free connection for 60s, something is really wrong
      //better raise an error than hang forever
      //see DefaultConnectionStrategy.getConnectionInternal()
      config.setConnectionTimeoutInMs(getConnectionTimeoutMs);
      config.setMaxConnectionsPerPartition(poolSize);
