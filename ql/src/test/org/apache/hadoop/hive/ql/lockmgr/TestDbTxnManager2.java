...
public class TestDbTxnManager2 {
   ...
   */
  @Test
  public void checkExpectedReadLocksNonStrict() throws Exception {
    dropTable(new String[] {"tab_acid", "tab_not_acid"});
    checkCmdOnDriver(driver.run("create table if not exists tab_acid (a int, b int) partitioned by (p string) " +
      "clustered by (a) into 2  buckets stored as orc TBLPROPERTIES ('transactional'='true')"));
    checkCmdOnDriver(driver.run("create table if not exists tab_not_acid (na int, nb int) partitioned by (np string) " +
    ...
    final HiveTxnManager txnMgr = null;
    // Test non-strict read-locking mode - the read locks are only obtained for the ACID side 
    conf.setBoolVar(HiveConf.ConfVars.HIVE_TXN_STRICT_READ_LOCKS, false);
    ...
    conf.setBoolVar(HiveConf.ConfVars.HIVE_TXN_STRICT_READ_LOCKS,
        HiveConf.ConfVars.HIVE_TXN_STRICT_READ_LOCKS.defaultBoolVal);
  }
