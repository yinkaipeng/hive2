  ...
  protected static boolean doVectorTest(String aggregationName, TypeInfo typeInfo,
    ...
    AggregationDesc aggrDesc =
        new AggregationDesc(
            aggregationName,
            evaluator,
            (ArrayList<ExprNodeDesc>) parameterList,
            /* distinct */ false,
            udafEvaluatorMode);

    VectorAggregateExpression vecAggrExpr =
        vectorizationContext.getAggregatorExpression(aggrDesc);
    if (vecAggrExpr == null) {
      Assert.fail(
    ...
    */
    ...
    for (short key = 0; key < maxKeyCount + 1; key++) {
      ...
      if (vectorAggregationBufferRow != null) {
        ...
        Object object = vecAggrExpr.evaluateOutput(aggregationBuffer);
        if (outputTypeInfo instanceof StructTypeInfo) {
          List<Object> structResult = new ArrayList<Object>(3);
          if (object == null) {
            structResult.add(null);
            structResult.add(null);
            structResult.add(null);
          } else {
            ...
            Object[] resultRow = (Object[]) object;

            ObjectInspector objectInspector0 =
                TypeInfoUtils.getStandardWritableObjectInspectorFromTypeInfo(
                    fieldTypeInfoList.get(0));
            Object copyResult0 =
                ObjectInspectorUtils.copyToStandardObject(
                    resultRow[0], objectInspector0, ObjectInspectorCopyOption.WRITABLE);
            structResult.add(copyResult0);

            ObjectInspector objectInspector1 =
                TypeInfoUtils.getStandardWritableObjectInspectorFromTypeInfo(
                    fieldTypeInfoList.get(1));
            Object copyResult1 =
                ObjectInspectorUtils.copyToStandardObject(
                    resultRow[1], objectInspector1, ObjectInspectorCopyOption.WRITABLE);
            structResult.add(copyResult1);
            ...
            if (resultRow.length == 2) {
              ...
              ObjectInspector objectInspector2 =
                  TypeInfoUtils.getStandardWritableObjectInspectorFromTypeInfo(
                      fieldTypeInfoList.get(2));
              Object copyResult2 =
                  ObjectInspectorUtils.copyToStandardObject(
                      resultRow[2], objectInspector2, ObjectInspectorCopyOption.WRITABLE);
              structResult.add(copyResult2);
            }
          }
          results[key] = structResult;
        } else {
          ObjectInspector objectInspector =
              TypeInfoUtils.getStandardWritableObjectInspectorFromTypeInfo(
                  outputTypeInfo);
