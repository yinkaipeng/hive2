...
set hive.optimize.index.filter=false;
select sum(hash(*)) from newtypesorc where d in (0.9, 0.22, 11.22);
...
set hive.optimize.index.filter=false;
select sum(hash(*)) from newtypesorc where d in (cast('0.9' as float), cast('0.22' as float), cast('11.22' as float));
