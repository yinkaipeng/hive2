...
public class HiveDruidRules {
  public static final RelOptRule EXPAND_SINGLE_DISTINCT_PROJECT_AGGREGATES_DRUID_RULE =
      new AggregateExpandDistinctAggregatesDruidRule(RelOptRule.operand(Aggregate.class, RelOptRule
          .operand(Project.class, RelOptRule.operand(DruidQuery.class, RelOptRule.none()), new RelOptRuleOperand[0])),
          HiveRelFactories.HIVE_BUILDER
      );

  public static final RelOptRule EXPAND_SINGLE_DISTINCT_AGGREGATES_DRUID_RULE =
      new AggregateExpandDistinctAggregatesDruidRule(
          RelOptRule.operand(Aggregate.class, RelOptRule.operand(DruidQuery.class, RelOptRule.none())),
          HiveRelFactories.HIVE_BUILDER
      );
  ...
  public static class AggregateExpandDistinctAggregatesDruidRule extends RelOptRule {
    ...
    public AggregateExpandDistinctAggregatesDruidRule(RelOptRuleOperand relOptRuleOperand, RelBuilderFactory relBuilderFactory) {
    ...
    @Override public void onMatch(RelOptRuleCall call) {
      ...
      final long numCountDistinct = aggregate.getAggCallList().stream().filter(
          aggregateCall -> aggregateCall.getAggregation().getKind().equals(SqlKind.COUNT) && aggregateCall.isDistinct())
          .count();
    ...
    private RelBuilder convertSingletonDistinct(RelBuilder relBuilder, Aggregate aggregate,
      ...
      for (AggregateCall aggCall : originalAggCalls) {
        ...
        if (!aggCall.isDistinct()) {
          final AggregateCall newCall = AggregateCall.create(aggCall.getAggregation(), false, aggCall.getArgList(), -1,
              ImmutableBitSet.of(bottomGroupSet).cardinality(), relBuilder.peek(), null, aggCall.name
          );
      ...
      relBuilder.push(aggregate
          .copy(aggregate.getTraitSet(), relBuilder.build(), false, ImmutableBitSet.of(bottomGroupSet), null,
      ...
      for (AggregateCall aggCall : originalAggCalls) {
        ...
        if (aggCall.isDistinct()) {
          ...
          newCall = AggregateCall
              .create(aggCall.getAggregation(), false, newArgList, -1, originalGroupSet.cardinality(),
          ...
          if (aggCall.getAggregation().getKind() == SqlKind.COUNT) {
            newCall = AggregateCall
                .create(new SqlSumEmptyIsZeroAggFunction(), false, newArgs, -1, originalGroupSet.cardinality(),
                    relBuilder.peek(), aggCall.getType(), aggCall.getName()
                );
          } else {
            newCall = AggregateCall
                .create(aggCall.getAggregation(), false, newArgs, -1, originalGroupSet.cardinality(), relBuilder.peek(),
      ...
      relBuilder.push(aggregate
          .copy(aggregate.getTraitSet(), relBuilder.build(), aggregate.indicator, ImmutableBitSet.of(topGroupSet), null,
              topAggregateCalls
          ));
      return relBuilder;
    }
