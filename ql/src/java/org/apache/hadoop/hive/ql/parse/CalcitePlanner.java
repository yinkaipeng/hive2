...
public class CalcitePlanner extends SemanticAnalyzer {
          ...
          if (this.conf.getBoolVar(HiveConf.ConfVars.HIVE_CBO_RETPATH_HIVEOP)) {
            ...
            if (oldHints.size() > 0) {
              if (getQB().getParseInfo().getHints() != null) {
                LOG.warn("Hints are not null in the optimized tree after CBO "
  ...
  boolean canCBOHandleAst(ASTNode ast, QB qb, PreCboCtx cboCtx) {
    ...
    boolean isSupportedType = qb.getIsQuery() || qb.isCTAS()
        || cboCtx.type == PreCboCtx.Type.INSERT || cboCtx.type == PreCboCtx.Type.MULTI_INSERT;
  ...
  private class CalcitePlannerAction implements Frameworks.PlannerAction<RelNode> {
    ...
    private boolean genSubQueryRelNode(QB qb, ASTNode node, RelNode srcRel, boolean forHavingClause,
      ...
      //disallow subqueries which HIVE doesn't currently support
        subqueryRestrictionCheck(qb, node, srcRel, forHavingClause, aliasToRel,
            corrScalarQueriesWithAgg, scalarQueriesWithAggNoWinNoGby);
    ...
    private RelNode genGBLogicalPlan(QB qb, RelNode srcRel) throws SemanticException {
      ...
      if (selExprList.getToken().getType() == HiveParser.TOK_SELECTDI
        ...
        if (node.getToken().getType() == HiveParser.TOK_ALLCOLREF) {
          ...
          srcRel = genSelectLogicalPlan(qb, srcRel, srcRel, true).getKey();
     ...
     * @throws SemanticException
     */
    private Pair<RelNode,RowResolver> genSelectLogicalPlan(QB qb, RelNode srcRel, RelNode starSrcRel, boolean isAllColRefRewrite)
        throws SemanticException {
    ...
    private RelNode genLogicalPlan(QB qb, boolean outerMostQB,
      ...
      Pair<RelNode, RowResolver> selPair = genSelectLogicalPlan(qb, srcRel, starSrcRel, false);
