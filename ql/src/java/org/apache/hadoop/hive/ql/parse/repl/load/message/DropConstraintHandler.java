...
public class DropConstraintHandler extends AbstractMessageHandler {
  ...
  public List<Task<? extends Serializable>> handle(Context context)
    ...
    Task<DDLWork> dropConstraintsTask = TaskFactory.get(new DDLWork(readEntitySet, writeEntitySet, dropConstraintsDesc), context.hiveConf, true);
    context.log.debug("Added drop constrain task : {}:{}", dropConstraintsTask.getId(), actualTblName);
    updatedMetadata.set(context.dmd.getEventTo().toString(), actualDbName, actualTblName, null);
    return Collections.<Task<? extends Serializable>>singletonList(dropConstraintsTask);
  }
}
