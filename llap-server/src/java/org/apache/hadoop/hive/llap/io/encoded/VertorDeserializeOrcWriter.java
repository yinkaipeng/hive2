...
class VertorDeserializeOrcWriter extends EncodingWriter implements Runnable {
  ...
  public void init(CacheWriter cacheWriter, Configuration conf) throws IOException {
    this.orcWriter = new WriterImpl(cacheWriter, null,
        SerDeEncodedDataReader.createOrcWriterOptions(destinationOi, conf));
  ...
  private static interface WriteOperation {
    boolean apply(WriterImpl writer, CacheWriter cacheWriter) throws IOException;
  }
  ...
  private static class VrbOperation implements WriteOperation {
    ...
    @Override
    public boolean apply(WriterImpl writer, CacheWriter cacheWriter) throws IOException {
